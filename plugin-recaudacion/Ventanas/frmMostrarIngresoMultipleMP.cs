...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.frmMostrarIngresoMultipleMP
...
public class frmMostrarIngresoMultipleMP : BaseForm
{
  private readonly BoletoGrupoMP grupo;
  private ExpIngresoMultipleMP expIngreso;
  ...
  public frmMostrarIngresoMultipleMP(BoletoGrupoMP gp)
  {
    this.InitializeComponent();
    this.grupo = gp;
    this.expIngreso = new ExpIngresoMultipleMP();
  ...
  private void AgregarBoleto(bool codiBar = false)
  {
    ...
    if (this.boletosAgregados.Exists((Predicate<BoletoCajaMP>) (x => x.NumeBole == this.mtbNumeBole.IntText && x.PeriBole == this.mtbPeriBole.IntText)))
    {
    ...
    else
    {
      try
      {
        ...
        BoletoCajaMP boletoCajaMp = this.expIngreso.ObtenerBoleto(this.mtbPeriBole.IntText, this.mtbNumeBole.IntText);
  ...
  private void RealizarPago()
  {
    try
    {
      if (!this.pagoEspecial)
        this.expIngreso.IngresarPagos(this.grupo.FeenAcpa, this.boletosAgregados, this.formasPagoUsadas.Where<FormaPagoMP>((Func<FormaPagoMP, bool>) (x => x.ImpoFopa > 0M)).ToList<FormaPagoMP>(), this.grupo.NumeAcpa, this.grupo.CodiEnre, this.grupo.FepaAcpa);
      else
        this.expIngreso.IngresarPagos(this.grupo.FeenAcpa, this.boletosAgregados, this.formasPagoUsadas.Where<FormaPagoMP>((Func<FormaPagoMP, bool>) (x => x.CodiTipa == 1)).ToList<FormaPagoMP>(), this.grupo.NumeAcpa, this.grupo.CodiEnre, this.grupo.FepaAcpa);
  ...
  {
    this.formasPagoUsadas.Find((Predicate<FormaPagoMP>) (x => x.CodiTipa == 2));
    if (this.formasPagoUsadas.Sum<FormaPagoMP>((Func<FormaPagoMP, Decimal>) (x => x.ImpoFopa)) - this.boletosAgregados.Sum<BoletoCajaMP>((Func<BoletoCajaMP, Decimal>) (x => x.TotaBole)) == 0M)
      this.RealizarPago();
    else
  ...
  private void InitializeComponent()
  {
    ...
    this.Name = nameof (frmMostrarIngresoMultipleMP);
    this.ShowIcon = false;
