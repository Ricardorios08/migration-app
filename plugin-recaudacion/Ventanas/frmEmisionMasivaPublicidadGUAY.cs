...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.frmEmisionMasivaPublicidadGUAY
...
public class frmEmisionMasivaPublicidadGUAY : FRMForm
{
  ...
  public frmEmisionMasivaPublicidadGUAY()
    : base("Emision masiva de Publicidad", "Emision masiva de Publicidad", typeof (ExpEmisionMasivaPublicidadGUAY))
  {
  ...
  public List<List<DTOBoletoAntenaGuay>> splitList<DTOBoletoAntenaGuay>(
    List<DTOBoletoAntenaGuay> locations,
    int nSize = 250)
  {
    List<List<DTOBoletoAntenaGuay>> boletoAntenaGuayListList = new List<List<DTOBoletoAntenaGuay>>();
    for (int index = 0; index < locations.Count; index += nSize)
      boletoAntenaGuayListList.Add(locations.GetRange(index, Math.Min(nSize, locations.Count - index)));
    return boletoAntenaGuayListList;
  }

  public List<List<DTOBoletoAntenaGuay>> splitListxCupones(
    List<DTOBoletoAntenaGuay> locations,
    int nSize = 250)
  {
    List<List<DTOBoletoAntenaGuay>> boletoAntenaGuayListList = new List<List<DTOBoletoAntenaGuay>>();
    ...
    for (int index = 0; index < locations.Count; index += val1)
      boletoAntenaGuayListList.Add(locations.GetRange(index, Math.Min(val1, locations.Count - index)));
    return boletoAntenaGuayListList;
  }
  ...
  private void ExportarPdfBut_Click(object sender, EventArgs e)
  {
    ...
    else
    {
      ...
      List<DTOBoletoAntenaGuay> boletoAntenaGuayList1 = new frmEsperar((Form) this).calcular<List<DTOBoletoAntenaGuay>>((Func<List<DTOBoletoAntenaGuay>>) (() => this.experto.obtenerLiquAntenas(row, this.DesdeText.IntText, this.HastaText.IntText)), "Consultado Datos...");
      List<DTOBoletoAntenaGuay> listaRepo = new List<DTOBoletoAntenaGuay>();
      int num = 0;
      int index = 0;
      foreach (DTOBoletoAntenaGuay comer in boletoAntenaGuayList1)
      {
      ...
      if (listaRepo.Count<DTOBoletoAntenaGuay>() > 250)
      {
        List<List<DTOBoletoAntenaGuay>> boletoAntenaGuayListList = this.splitList<DTOBoletoAntenaGuay>(listaRepo);
        FolderBrowserDialog dialog = new FolderBrowserDialog();
        if (dialog.ShowDialog((IWin32Window) this) == DialogResult.OK)
        {
          foreach (List<DTOBoletoAntenaGuay> boletoAntenaGuayList2 in boletoAntenaGuayListList)
          {
            List<DTOBoletoAntenaGuay> l = boletoAntenaGuayList2;
            irepBoletoAntenaGuay bole = new irepBoletoAntenaGuay();
            ((XtraReportBase) bole).DataSource = (object) l;
            bole.createReport((Action<int, string>) null, false);
            PdfExportOptions pdfExportOptions = new PdfExportOptions();
            new frmEsperar((Form) this).procesar((Action) (() => bole.ExportToPdf($"{dialog.SelectedPath}\\comercios ({l[0].OrdeLian.ToString()}-{l[l.Count - 1].OrdeLian.ToString()}).pdf", (PdfExportOptions) null)), "Exportando..");
          }
        }
  ...
  public void agregarQrVinculado(List<DTOBoletoAntenaGuay> listaRepo)
  {
    foreach (DTOBoletoAntenaGuay boletoAntenaGuay in listaRepo)
    {
      string str1 = "";
      foreach (CuponAntena cupone in boletoAntenaGuay.cupones)
        str1 = $"{str1}{cupone.NumeBole.ToString()}-";
      string str2 = str1.TrimEnd('-');
      boletoAntenaGuay.EpagosQrVinc = $"https://vps.infogov.com.ar/rentas/guaymallen/V1/index.php/api/epagos/opcionespago/{str2}/{boletoAntenaGuay.PeriBole.ToString()}";
    }
  }
  ...
  {
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (frmEmisionMasivaPublicidadGUAY));
    ...
    this.separator1.Size = new Size(11185, 2);
    ...
    this.Name = nameof (frmEmisionMasivaPublicidadGUAY);
    this.Text = "Emision Masiva de Publicidad";
