...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.frmEmisionMasivaTasas
...
public class frmEmisionMasivaTasas : FRMForm
{
  ...
  private MetroButton butGenerarQrEpagos;
  ...
  public frmEmisionMasivaTasas()
    : base("Emision masiva de tasas", "Emision masiva de tasas", typeof (ExpEmisionMasivaTasas))
  {
  ...
  {
    int num = this.experto.sugiereOrdenNew((EmisionMasivaTasas) ((BaseView) this.gridView1).GetRow(((ColumnView) this.gridView1).GetSelectedRows()[0]));
  ...
  public List<List<DTOTasasRepo>> splitList<DTOTasasRepo>(List<DTOTasasRepo> locations, int nSize = 250)
  {
    List<List<DTOTasasRepo>> dtoTasasRepoListList = new List<List<DTOTasasRepo>>();
    for (int index = 0; index < locations.Count; index += nSize)
      dtoTasasRepoListList.Add(locations.GetRange(index, Math.Min(nSize, locations.Count - index)));
  ...
  private void ExportarPdfBut_Click(object sender, EventArgs e)
  {
    if (this.validar())
      return;
    EmisionMasivaTasas row = (EmisionMasivaTasas) ((BaseView) this.gridView1).GetRow(((ColumnView) this.gridView1).GetSelectedRows()[0]);
    if (this.HastaText.IntText > row.CantOrdeLita)
    {
      new frmMensaje((Form) this).mostrarError("Generación fuera de Rango.");
    }
    else
    {
      this.ExportarPdfBut.Enabled = this.butPdfMail.Enabled = this.butGenerarQrEpagos.Enabled = false;
      List<DTOTasasRepo> dtoTasasRepoList1 = new frmEsperar((Form) this).calcular<List<DTOTasasRepo>>((Func<List<DTOTasasRepo>>) (() => this.experto.obtenerTasas(row, this.DesdeText.IntText, this.HastaText.IntText)), "Consultado Datos...");
      List<DTOTasasRepo> listaRepo = new List<DTOTasasRepo>();
      int num = 0;
      int index = 0;
      foreach (DTOTasasRepo tasa in dtoTasasRepoList1)
      {
      ...
      new frmEsperar((Form) this).procesar((Action) (() => this.experto.ProcesarAgregarFiduciario(listaRepo)), "Consultando Fiduciarios...");
      if (listaRepo.Count<DTOTasasRepo>() > 250)
      {
        List<List<DTOTasasRepo>> dtoTasasRepoListList = this.splitList<DTOTasasRepo>(listaRepo);
        FolderBrowserDialog dialog = new FolderBrowserDialog();
        if (dialog.ShowDialog((IWin32Window) this) == DialogResult.OK)
        {
          foreach (List<DTOTasasRepo> dtoTasasRepoList2 in dtoTasasRepoListList)
          {
            List<DTOTasasRepo> l = dtoTasasRepoList2;
            irepBoletoTasa bole = new irepBoletoTasa();
      ...
      this.ExportarPdfBut.Enabled = this.butPdfMail.Enabled = this.butGenerarQrEpagos.Enabled = true;
    }
  }
  ...
  private void butPdfMail_Click(object sender, EventArgs e)
  {
    ...
    {
      this.ExportarPdfBut.Enabled = this.butPdfMail.Enabled = this.butGenerarQrEpagos.Enabled = false;
      List<DTOTasasRepo> dtoTasasRepoList1 = new frmEsperar((Form) this).calcular<List<DTOTasasRepo>>((Func<List<DTOTasasRepo>>) (() => this.experto.obtenerTasasParaCorreo(row, this.DesdeText.IntText, this.HastaText.IntText)), "Consultado Datos...");
      List<DTOTasasRepo> listaRepo = new List<DTOTasasRepo>();
      List<DTOTasasRepo> dtoTasasRepoList2 = new List<DTOTasasRepo>();
      int num = 0;
      int index = 0;
      List<DTOTasasRepo> dtoTasasRepoList3 = this.experto.CuentasConMail();
      foreach (DTOTasasRepo dtoTasasRepo1 in dtoTasasRepoList1)
      {
        foreach (DTOTasasRepo dtoTasasRepo2 in dtoTasasRepoList3)
        {
          if (dtoTasasRepo1.CodiInmu.ToString() == dtoTasasRepo2.CuenCtct)
          {
            dtoTasasRepo1.Email = dtoTasasRepo2.Email;
            dtoTasasRepoList2.Add(dtoTasasRepo1);
          }
        }
      }
      foreach (DTOTasasRepo tasa in dtoTasasRepoList2)
      {
        if (tasa.CodiInmu != num)
        {
          num = tasa.CodiInmu;
      ...
      if (dialog.ShowDialog((IWin32Window) this) == DialogResult.OK)
      {
        ...
        {
          DTOTasasRepo l = dtoTasasRepo;
          irepBoletoTasa bole = new irepBoletoTasa();
          ((XtraReportBase) bole).DataSource = (object) new List<DTOTasasRepo>()
          {
            l
          };
          bole.createReport((Action<int, string>) null, false);
          PdfExportOptions pdfExportOptions = new PdfExportOptions();
          new frmEsperar((Form) this).procesar((Action) (() => bole.ExportToPdf($"{dialog.SelectedPath}\\1-{l.CodiInmu.ToString()}.pdf", (PdfExportOptions) null)), "Exportando..");
        }
      }
      new frmMensaje((Form) this).mostrarConfirmación("Proceso terminado");
      this.ExportarPdfBut.Enabled = this.butPdfMail.Enabled = this.butGenerarQrEpagos.Enabled = true;
  ...
  private void butGenerarQrEpagos_Click(object sender, EventArgs e)
  {
    ...
    if (this.validar())
      return;
    EmisionMasivaTasas Tasas = (EmisionMasivaTasas) ((BaseView) this.gridView1).GetRow(((ColumnView) this.gridView1).FocusedRowHandle);
    if (Tasas.PendEpQr == 0)
    {
    ...
    {
      List<DTOTasasRepo> locations = new frmEsperar((Form) this).calcular<List<DTOTasasRepo>>((Func<List<DTOTasasRepo>>) (() => this.experto.ObtenerTasaparaEpagosQr(Tasas, this.DesdeText.IntText, this.HastaText.IntText)), "Consultado Datos...");
      ...
      else
      {
        ...
        List<List<DTOTasasRepo>> ListaPartida = this.splitListxCupones(locations, 50);
        DateTime dateTime = Misc.Now();
        new frmEsperar((Form) this).calcular<List<List<DTOTasasRepo>>>((Func<List<List<DTOTasasRepo>>>) (() => this.experto.ProcesarQrEpagosLotes(ListaPartida)), "Generando Qr Epagos...");
        new frmEsperar((Form) this).calcular<bool>((Func<bool>) (() => this.experto.ProcesarActualizarQrEpagos(ListaPartida)), "Actualizando Emision con Qr Epagos...");
        new frmMensaje((Form) this).mostrarInformación("Duración: " + (Misc.Now() - dateTime).ToString());
        this.ExportarPdfBut.Enabled = this.butPdfMail.Enabled = this.butGenerarQrEpagos.Enabled = true;
      }
    }
  }

  private void agregarProceso(List<List<DTOTasasRepo>> lotes)
  {
    ucGenerarQR ucGenerarQr = new ucGenerarQR();
    this.flow.Controls.Add((Control) ucGenerarQr);
    ucGenerarQr.iniciarProceso(lotes);
  }
  ...
  {
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (frmEmisionMasivaTasas));
    this.Filtro = new SimpleFilterBar();
    this.metroPanel1 = new MetroPanel();
    this.butGenerarQrEpagos = new MetroButton();
    ...
    this.separator1.Size = new Size(7649, 2);
    ...
    this.metroPanel1.Controls.Add((Control) this.butGenerarQrEpagos);
    ...
    this.butGenerarQrEpagos.Location = new Point(562, 126);
    this.butGenerarQrEpagos.Name = "butGenerarQrEpagos";
    this.butGenerarQrEpagos.Size = new Size(149, 23);
    this.butGenerarQrEpagos.TabIndex = 9;
    this.butGenerarQrEpagos.Text = "GENERAR QRs EPAGOS";
    this.butGenerarQrEpagos.Visible = false;
    this.butGenerarQrEpagos.Click += new EventHandler(this.butGenerarQrEpagos_Click);
    ...
    this.butVolver.Click += new EventHandler(this.butVolver_Click);
    ...
    this.Name = nameof (frmEmisionMasivaTasas);
