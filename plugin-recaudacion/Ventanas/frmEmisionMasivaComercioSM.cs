...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.frmEmisionMasivaComercioSM
...
public class frmEmisionMasivaComercioSM : FRMForm
{
  ...
  public frmEmisionMasivaComercioSM()
    : base("Emision Masiva Comercio", "Emision Masiva Comercio", typeof (ExpEmisionMasivaComercioSM))
  {
  ...
  public List<List<DTOBoletoComerSM>> splitList<DTOBoletoComerSM>(
    List<DTOBoletoComerSM> locations,
    int nSize = 250)
  {
    List<List<DTOBoletoComerSM>> dtoBoletoComerSmListList = new List<List<DTOBoletoComerSM>>();
    for (int index = 0; index < locations.Count; index += nSize)
      dtoBoletoComerSmListList.Add(locations.GetRange(index, Math.Min(nSize, locations.Count - index)));
    return dtoBoletoComerSmListList;
  }

  public List<List<DTOBoletoComerSM>> splitListxCupones(List<DTOBoletoComerSM> locations, int nSize = 250)
  {
    List<List<DTOBoletoComerSM>> dtoBoletoComerSmListList = new List<List<DTOBoletoComerSM>>();
    int val1 = nSize / locations[0].cupones.Count;
    for (int index = 0; index < locations.Count; index += val1)
      dtoBoletoComerSmListList.Add(locations.GetRange(index, Math.Min(val1, locations.Count - index)));
    return dtoBoletoComerSmListList;
  }
  ...
  {
    List<DTOBoletoComerSM> dtoBoletoComerSmList1 = this.buscarLiquidacionComer();
    short op = (short) this.ComboBoxOpcion.SelectedIndex;
    if (dtoBoletoComerSmList1.Count<DTOBoletoComerSM>() > 250)
    {
      List<List<DTOBoletoComerSM>> dtoBoletoComerSmListList = this.splitList<DTOBoletoComerSM>(dtoBoletoComerSmList1);
      FolderBrowserDialog dialog = new FolderBrowserDialog();
      if (dialog.ShowDialog((IWin32Window) this) == DialogResult.OK)
      {
        foreach (List<DTOBoletoComerSM> dtoBoletoComerSmList2 in dtoBoletoComerSmListList)
        {
          List<DTOBoletoComerSM> l = dtoBoletoComerSmList2;
          irepBoletoComerSM bole = new irepBoletoComerSM(op);
          ((XtraReportBase) bole).DataSource = (object) l;
          bole.createReport((Action<int, string>) null, false);
          PdfExportOptions pdfExportOptions = new PdfExportOptions();
          new frmEsperar((Form) this).procesar((Action) (() => bole.ExportToPdf($"{dialog.SelectedPath}\\{(op == (short) 0 ? "comercio (" : "variable (")}{l[0].OrdeLico.ToString()}-{l[l.Count - 1].OrdeLico.ToString()}).pdf", (PdfExportOptions) null)), "Exportando..");
        }
      }
    ...
    {
      irepBoletoComerSM bole = new irepBoletoComerSM(op);
      ((XtraReportBase) bole).DataSource = (object) dtoBoletoComerSmList1;
      bole.createReport((Action<int, string>) null, false);
  ...
  {
    irepBoletoComerSM bole = new irepBoletoComerSM((short) 2);
    ...
    dtoBoletoComerSm.PeriBole = PARAMS.Periodo;
    dtoBoletoComerSm.CucuPers = "";
    dtoBoletoComerSmList.Add(dtoBoletoComerSm);
    ((XtraReportBase) bole).DataSource = (object) dtoBoletoComerSmList;
    bole.createReport((Action<int, string>) null, false);
    SaveFileDialog dialog = new SaveFileDialog();
    dialog.AddExtension = true;
    dialog.AutoUpgradeEnabled = true;
  ...
  {
    List<DTOBoletoComerSM> listaRepo = this.buscarLiquidacionComer();
  ...
  private List<DTOBoletoComerSM> buscarLiquidacionComer()
  {
    List<DTOBoletoComerSM> listaRepo = new List<DTOBoletoComerSM>();
    if (this.validar())
      return listaRepo;
    EmisionMasivaComercioSM row = (EmisionMasivaComercioSM) ((BaseView) this.gridView1).GetRow(((ColumnView) this.gridView1).GetSelectedRows()[0]);
    ...
    List<DTOBoletoComerSM> dtoBoletoComerSmList = new frmEsperar((Form) this).calcular<List<DTOBoletoComerSM>>((Func<List<DTOBoletoComerSM>>) (() => this.experto.obtenerComerciosSM(row, this.DesdeText.IntText, this.HastaText.IntText)), "Consultado Datos...");
    int num = 0;
    int index = 0;
    foreach (DTOBoletoComerSM come in dtoBoletoComerSmList)
    {
      if (come.CodiCome != num)
      {
        num = come.CodiCome;
        listaRepo.Add(come);
        listaRepo[index].cupones = new List<DTOBoletoComerSM.CuponComerSM>();
        listaRepo[index].cupones.Add(this.experto.obtenerCuponSM(row, come, come.TipoVenc == 2));
        ++index;
      }
      else
        listaRepo[index - 1].cupones.Add(this.experto.obtenerCuponSM(row, come, come.TipoVenc == 2));
    }
  ...
  public void ExportarEmailgPdf(List<DTOBoletoComerSM> listaRepo, string rutaFinal)
  {
    foreach (DTOBoletoComerSM dtoBoletoComerSm in listaRepo)
    {
      irepBoletoComerSM irepBoletoComerSm = new irepBoletoComerSM((short) 0);
      ((XtraReportBase) irepBoletoComerSm).DataSource = (object) new List<DTOBoletoComerSM>()
      {
        dtoBoletoComerSm
      };
      irepBoletoComerSm.createReport((Action<int, string>) null, false);
      PdfExportOptions pdfExportOptions = new PdfExportOptions();
      irepBoletoComerSm.ExportToPdf($"{rutaFinal}\\2-{dtoBoletoComerSm.CodiCome.ToString()}.pdf", (PdfExportOptions) null);
    }
  }
  ...
  {
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (frmEmisionMasivaComercioSM));
    ...
    this.separator1.Size = new Size(12069, 2);
    ...
    this.butGenerarPdfEmail.TabIndex = 11;
    ...
    this.Name = nameof (frmEmisionMasivaComercioSM);
    this.Text = "Emision Masiva de Comercio";
