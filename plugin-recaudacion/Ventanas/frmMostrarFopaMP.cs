...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.frmMostrarFopaMP
...
public class frmMostrarFopaMP : BaseForm
{
  private readonly List<FormaPagoMP> formasPago;
  private readonly List<FormaPagoMP> formasPagoDisponibles;
  private readonly List<FormaPagoMP> formasPagoUsadas;
  ...
  private FormaPagoMP TipaSeleccionada;
  private FormaPagoMP FopaSeleccionada;
  private readonly ExpBoletoGrupoMP expGrupo;
  private List<BoletoAsociadoMP> boletos;
    ...
    int comb)
  {
    ...
    try
    {
      this.expGrupo = new ExpBoletoGrupoMP();
      ...
      foreach (FormaPagoMP formaPagoMp in this.formasPago)
      {
        FormaPagoMP fp = formaPagoMp;
        if (!this.formasPagoUsadas.Exists((Predicate<FormaPagoMP>) (x => x.CodiTipa == fp.CodiTipa)))
  ...
  private void BuscarBoletosAsociados(bool mostrarResto)
  {
    ...
    this.lblImpoPago.Text = $"El importe restante a redistribuir es: {this.boletos.Sum<BoletoAsociadoMP>((Func<BoletoAsociadoMP, Decimal>) (x => x.TotaBole)):C2}";
  ...
  private void btnConfirmar_Click(object sender, EventArgs e)
  {
    ...
    if (string.IsNullOrEmpty(this.MoanBole) || this.MoanBole.Length <= 10)
      new frmMensaje((Form) this).mostrarError("Debe ingresar un motivo entre 10 y 30 carácteres.");
    else if (this.combinacion != 4)
      this.RealizarCambio();
    else if (this.formasPagoUsadas.Sum<FormaPagoMP>((Func<FormaPagoMP, Decimal>) (x => x.ImpoFopa)) - this.boletos.Sum<BoletoAsociadoMP>((Func<BoletoAsociadoMP, Decimal>) (x => x.TotaBole)) == 0M)
      this.RealizarCambio();
    else
  ...
  {
    if (this.expGrupo.EliminarPago(this.FeenAcpa, this.NumeAcpa, this.PeriBole, this.NumeBole, this.MoanBole, this.TotaBole, this.combinacion, this.PeriFopa, this.NumeFopa, this.formasPagoUsadas.Where<FormaPagoMP>((Func<FormaPagoMP, bool>) (x => x.ImpoFopa > 0M)).ToList<FormaPagoMP>()))
    {
  ...
  private void CalcularFormasPago()
  {
    if (this.formasPagoUsadas.Count<FormaPagoMP>() > 1)
    {
      ...
      if (num1 > 0)
      {
        Decimal num2 = this.boletos.Sum<BoletoAsociadoMP>((Func<BoletoAsociadoMP, Decimal>) (x => x.TotaBole)) - this.formasPagoUsadas.Sum<FormaPagoMP>((Func<FormaPagoMP, Decimal>) (x => x.ImpoFopa));
        if (num2 > 0M)
        {
          ...
          EnumerableExtensions.ForEach<FormaPagoMP>(this.formasPagoUsadas.Where<FormaPagoMP>((Func<FormaPagoMP, bool>) (x => x.ImpoFopa == 0M)), (Action<FormaPagoMP>) (x => x.ImpoFopa = Decimal.Round(restoAFila, 2)));
          Decimal num3 = Decimal.Round(this.boletos.Sum<BoletoAsociadoMP>((Func<BoletoAsociadoMP, Decimal>) (x => x.TotaBole)) - this.formasPagoUsadas.Sum<FormaPagoMP>((Func<FormaPagoMP, Decimal>) (x => x.ImpoFopa)), 2);
          if (num3 != 0M)
  ...
  private void viewTipas_RowClick(object sender, RowClickEventArgs e)
  {
    if (this.combinacion != 4)
      return;
    this.TipaSeleccionada = (FormaPagoMP) ((BaseView) this.viewTipas).GetRow(((ColumnView) this.viewTipas).GetSelectedRows()[0]);
    if (this.TipaSeleccionada == null || this.formasPagoDisponibles.Count <= 0)
      return;
    Decimal num = this.boletos.Sum<BoletoAsociadoMP>((Func<BoletoAsociadoMP, Decimal>) (x => x.TotaBole)) - this.formasPagoUsadas.Sum<FormaPagoMP>((Func<FormaPagoMP, Decimal>) (x => x.ImpoFopa));
    ...
    this.CalcularPago();
    this.RefrescarGridViews();
    this.TipaSeleccionada = (FormaPagoMP) null;
  }
  ...
  {
    Decimal num1 = 0.00M;
    if (this.boletos.Count<BoletoAsociadoMP>() > 0)
      num1 = Decimal.Round(this.boletos.Sum<BoletoAsociadoMP>((Func<BoletoAsociadoMP, Decimal>) (x => x.TotaBole)), 2);
  ...
  private void InitializeComponent()
  {
    ...
    this.Name = nameof (frmMostrarFopaMP);
    this.ShowIcon = false;
