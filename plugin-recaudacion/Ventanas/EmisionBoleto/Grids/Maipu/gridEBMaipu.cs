...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.EmisionBoleto.Grids.Maipu.gridEBMaipu
...
using InfoGov.Plugins.Recaudacion.Ventanas.EmisionBoleto.Modals;
using InfoGov.Utils;
using System;
...
#nullable disable
namespace InfoGov.Plugins.Recaudacion.Ventanas.EmisionBoleto.Grids.Maipu;

public class gridEBMaipu : EmisionBoletoBase, IGridEmisionBoleto
{
  private readonly ExpCCMaipu experto;
  private readonly List<CCMaipu> lista;
  private List<CCMaipu> deudaSeleccionada;
  private Decimal impoPagoCuenta = 0M;
  private bool EsPagoACuenta = false;
  private Decimal totalSeleccionado = 0M;
  private readonly ExpEBMaipu expEB;
  private IContainer components = (IContainer) null;
  private MetroPanel panDetaSuperior;
  private MetroCheckBox chkPagoCuenta;
  private MetroLabel lblImpoPagoCuenta;
  private MetroDecimalTextBox mdbPagoCuenta;
  ...
  private SimpleButton btnGenerarBoleto;
  ...
  public override void Limpiar()
  {
    ...
    this.EjecutarEnUI(new Action((object) this, __methodptr(\u003CLimpiar\u003Eg__RealizarLimpieza\u007C8_0)));
  }
  ...
  public override void Buscar(CtaCte cuenta)
  {
    ...
    {
      this.experto.BuscarDeuda(this.lista, cuenta, ExpCCMaipu.DeudaPara.EmisionBoleto);
  ...
  private bool ValidarDeudaSeleccionada()
  {
    ...
    if (this.deudaSeleccionada == null || this.deudaSeleccionada.Count == 0)
      errores.Add("No hay deuda seleccionada.");
    else if (this.EsPagoACuenta && this.impoPagoCuenta <= 0M)
      errores.Add("El importe de pago a cuenta debe \nser mayor a cero.");
    if (errores.Count > 0)
      new dlgErrorList(this.ParentForm).mostarError(errores);
  ...
  private void ActivarPanelCentral(bool activar) => this.panCentral.Enabled = activar;

  private void mdbPagoCuenta_Leave(object sender, EventArgs e)
  {
    if (!this.mdbPagoCuenta.Enabled)
      return;
    this.ActivarPanelCentral(this.mdbPagoCuenta.DecimalText > 0M);
    this.impoPagoCuenta = this.mdbPagoCuenta.DecimalText;
  }

  private void chkPagoCuenta_CheckedChanged(object sender, EventArgs e)
  {
    if (!this.TerminoCarga)
      return;
    this.mdbPagoCuenta.Text = string.Empty;
    this.impoPagoCuenta = 0M;
    this.EsPagoACuenta = false;
    this.lblImpoPagoCuenta.Visible = this.mdbPagoCuenta.Visible = this.mdbPagoCuenta.Enabled = this.chkPagoCuenta.Checked;
    if (this.mdbPagoCuenta.Enabled)
      this.mdbPagoCuenta.Focus();
    else
      this.ActivarPanelCentral(true);
    ((ColumnView) this.gridViewResumen).FocusInvalidRow();
  }

  private void btnGenerarBoleto_Click(object sender, EventArgs e)
  {
    if (!this.ValidarDeudaSeleccionada())
      return;
    DTOEmiBole dto = new DTOEmiBole();
    CtaCte ctaCte = ((frmEmisionBoleto) this.ParentForm).ctacte?.GenerarNuevaInstancia();
    if (ctaCte != null)
    {
      ctaCte.FeveCtctBole = this.expEB.CalcularFechaVencimiento(ctaCte.FeveCtct, (short) 3);
      dto.DatosCuenta = ctaCte;
      dto.EsPagoACuenta = this.EsPagoACuenta;
      dto.ImpoPagoCuenta = dto.EsPagoACuenta ? this.impoPagoCuenta : 0M;
      dto.DeudaSeleccionada = this.deudaSeleccionada.Cast<IReporteBoleto>().ToList<IReporteBoleto>();
      dto.DatosBoleto = this.expEB.ObtenerDatosBoleto(dto.DatosCuenta.CodiOfic, dto.DatosCuenta.CuenCtct);
      dto.Titulo = "BOLETO DE DEUDA";
      if (new mdlResumen(dto, this.expEB).ShowDialog((IWin32Window) this.ParentForm) != DialogResult.OK)
        return;
      ToastHelper.ShowOrFallback((Form) null, "Boleto generado correctamente.", ToastHelper.ToastType.Success, ToastHelper.ToastPosition.TopRight);
      ((frmEmisionBoleto) this.ParentForm).LimpiarControles();
    }
    else
      ToastHelper.ShowOrFallback((Form) null, "No se encontraron los datos de la cuenta corriente. Intente buscar nuevamente.", ToastHelper.ToastType.Danger, ToastHelper.ToastPosition.TopRight);
  }
  ...
  private void gridViewGrilla_SelectionChanged(object sender, SelectionChangedEventArgs e)
  {
    if (!this.chkPagoCuenta.Checked || !(this.impoPagoCuenta > 0M))
      return;
    int[] selectedRows = ((ColumnView) this.gridViewGrilla).GetSelectedRows();
    Decimal num1 = 0M;
    foreach (int num2 in selectedRows)
      num1 += ((DetaCCMaipu) ((BaseView) this.gridViewGrilla).GetRow(num2)).TotaCtct;
    if (num1 >= this.impoPagoCuenta)
    {
      this.EsPagoACuenta = true;
      this.ActivarPanelCentral(false);
      this.btnGenerarBoleto_Click((object) this.btnGenerarBoleto, (EventArgs) null);
    }
    else
    {
      this.EsPagoACuenta = false;
  ...
  {
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (gridEBMaipu));
    this.panDetaSuperior = new MetroPanel();
    this.txtDeudaSelec = new MetroTextBox();
    this.metroLabel3 = new MetroLabel();
    this.btnGenerarBoleto = new SimpleButton();
    this.mdbPagoCuenta = new MetroDecimalTextBox();
    this.lblImpoPagoCuenta = new MetroLabel();
    this.chkPagoCuenta = new MetroCheckBox();
    ...
    this.panDetaSuperior.SuspendLayout();
    ...
    this.SuspendLayout();
    this.panDetaSuperior.Controls.Add((Control) this.txtDeudaSelec);
    this.panDetaSuperior.Controls.Add((Control) this.metroLabel3);
    this.panDetaSuperior.Controls.Add((Control) this.btnGenerarBoleto);
    this.panDetaSuperior.Controls.Add((Control) this.mdbPagoCuenta);
    this.panDetaSuperior.Controls.Add((Control) this.lblImpoPagoCuenta);
    this.panDetaSuperior.Controls.Add((Control) this.chkPagoCuenta);
    this.panDetaSuperior.Dock = DockStyle.Top;
    this.panDetaSuperior.Location = new Point(0, 0);
    this.panDetaSuperior.Name = "panDetaSuperior";
    this.panDetaSuperior.Size = new Size(1360, 79);
    this.panDetaSuperior.TabIndex = 0;
    ...
    this.txtDeudaSelec.Location = new Point(924, 36);
    ...
    this.txtDeudaSelec.TabIndex = 10;
    this.txtDeudaSelec.TabStop = false;
    this.txtDeudaSelec.TextAlign = HorizontalAlignment.Right;
    this.metroLabel3.AutoSize = false;
    this.metroLabel3.Location = new Point(948, 6);
    this.metroLabel3.Name = "metroLabel3";
    this.metroLabel3.Size = new Size((int) sbyte.MaxValue, 19);
    this.metroLabel3.TabIndex = 9;
    this.metroLabel3.Text = "Deuda Seleccionada";
    ((BaseStyleControl) this.btnGenerarBoleto).Appearance.Font = new Font("Calibri", 10f, FontStyle.Bold);
    ((BaseStyleControl) this.btnGenerarBoleto).Appearance.Options.UseFont = true;
    ((BaseButton) this.btnGenerarBoleto).ButtonStyle = (BorderStyles) 3;
    ((ImageOptions) this.btnGenerarBoleto.ImageOptions).Image = (Image) componentResourceManager.GetObject("btnGenerarBoleto.ImageOptions.Image");
    ((Control) this.btnGenerarBoleto).Location = new Point(663, 21);
    ((Control) this.btnGenerarBoleto).Name = "btnGenerarBoleto";
    ((Control) this.btnGenerarBoleto).Size = new Size(45, 34);
    ((Control) this.btnGenerarBoleto).TabIndex = 8;
    ((Control) this.btnGenerarBoleto).Click += new EventHandler(this.btnGenerarBoleto_Click);
    this.mdbPagoCuenta.CausesValidation = false;
    this.mdbPagoCuenta.Enabled = false;
    this.mdbPagoCuenta.Location = new Point(1231, 36);
    this.mdbPagoCuenta.MaxLength = 11;
    this.mdbPagoCuenta.Name = "mdbPagoCuenta";
    this.mdbPagoCuenta.Size = new Size(113, 23);
    this.mdbPagoCuenta.TabIndex = 2;
    this.mdbPagoCuenta.TextAlign = HorizontalAlignment.Right;
    this.mdbPagoCuenta.Visible = false;
    this.mdbPagoCuenta.Leave += new EventHandler(this.mdbPagoCuenta_Leave);
    this.lblImpoPagoCuenta.AutoSize = false;
    this.lblImpoPagoCuenta.Enabled = false;
    this.lblImpoPagoCuenta.Location = new Point(1162, 36);
    this.lblImpoPagoCuenta.Name = "lblImpoPagoCuenta";
    this.lblImpoPagoCuenta.Size = new Size(57, 19);
    this.lblImpoPagoCuenta.TabIndex = 5;
    this.lblImpoPagoCuenta.Text = "Importe";
    this.lblImpoPagoCuenta.Visible = false;
    this.chkPagoCuenta.AutoSize = false;
    this.chkPagoCuenta.Location = new Point(1231, 6);
    this.chkPagoCuenta.Name = "chkPagoCuenta";
    this.chkPagoCuenta.Size = new Size(113, 19);
    this.chkPagoCuenta.TabIndex = 1;
    this.chkPagoCuenta.Text = "Pago A Cuenta";
    this.chkPagoCuenta.Visible = false;
    this.chkPagoCuenta.CheckedChanged += new EventHandler(this.chkPagoCuenta_CheckedChanged);
    this.panCentral.Controls.Add((Control) this.panDetalles);
    this.panCentral.Controls.Add((Control) this.panSuperior);
    this.panCentral.Dock = DockStyle.Fill;
    this.panCentral.Location = new Point(0, 79);
    this.panCentral.Name = "panCentral";
    this.panCentral.Size = new Size(1360, 591);
    ...
    this.panDetalles.Location = new Point(0, 154);
    this.panDetalles.Name = "panDetalles";
    this.panDetalles.Size = new Size(1360, 437);
    this.panDetalles.TabIndex = 3;
    ...
    ((Control) this.gridControlGrilla).Size = new Size(1360, 418);
    ((Control) this.gridControlGrilla).TabIndex = 3;
    ...
    ((ColumnView) this.gridViewGrilla).SelectionChanged += new SelectionChangedEventHandler(this.gridViewGrilla_SelectionChanged);
    ...
    this.metroLabel2.Size = new Size(1360, 19);
    ...
    this.panSuperior.Size = new Size(1360, 154);
    ...
    ((Control) this.gridControlResumen).Size = new Size(1360, 135);
    ...
    this.metroLabel1.Size = new Size(1360, 19);
    ...
    this.Controls.Add((Control) this.panDetaSuperior);
    this.Name = nameof (gridEBMaipu);
    this.Size = new Size(1360, 670);
    this.panDetaSuperior.ResumeLayout(false);
