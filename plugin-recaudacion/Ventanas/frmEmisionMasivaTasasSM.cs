...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.frmEmisionMasivaTasasSM
...
public class frmEmisionMasivaTasasSM : FRMForm
{
  private BindingSource bsEmison = new BindingSource();
  private ExpEmisionMasivaTasasSM experto = new ExpEmisionMasivaTasasSM();
  ...
  public frmEmisionMasivaTasasSM()
    : base("Emision masiva de tasas", "Emision masiva de tasas", typeof (ExpEmisionMasivaTasas))
  {
    ...
    if (!(Misc.Usuario == "root"))
      return;
    this.ExportarPdfBut.Visible = true;
  ...
  {
    this.ExportarPdfBut.Enabled = this.btnGenerarFijo.Enabled = this.butGenerarPdfEmail.Enabled = false;
    List<DTOBoletoInmuSM> dtoBoletoInmuSmList1 = this.buscarLiquiedacionInmu();
    short op = (short) this.ComboBoxOpcion.SelectedIndex;
    if (dtoBoletoInmuSmList1.Count<DTOBoletoInmuSM>() > 250)
    {
      List<List<DTOBoletoInmuSM>> dtoBoletoInmuSmListList = this.splitList<DTOBoletoInmuSM>(dtoBoletoInmuSmList1);
      FolderBrowserDialog dialog = new FolderBrowserDialog();
      if (dialog.ShowDialog((IWin32Window) this) == DialogResult.OK)
      {
        foreach (List<DTOBoletoInmuSM> dtoBoletoInmuSmList2 in dtoBoletoInmuSmListList)
        {
          List<DTOBoletoInmuSM> l = dtoBoletoInmuSmList2;
          irepBoletoInmuSM bole = new irepBoletoInmuSM(op);
          ((XtraReportBase) bole).DataSource = (object) l;
          bole.createReport((Action<int, string>) null, false);
          PdfExportOptions pdfExportOptions = new PdfExportOptions();
          new frmEsperar((Form) this).procesar((Action) (() => bole.ExportToPdf($"{dialog.SelectedPath}\\{(op == (short) 0 ? "tasas (" : "variable (")}{l[0].OrdeLita.ToString()}-{l[l.Count - 1].OrdeLita.ToString()}).pdf", (PdfExportOptions) null)), "Exportando..");
        }
      }
    ...
    {
      irepBoletoInmuSM bole = new irepBoletoInmuSM(op);
      ((XtraReportBase) bole).DataSource = (object) dtoBoletoInmuSmList1;
      bole.createReport((Action<int, string>) null, false);
  ...
  {
    irepBoletoInmuSM bole = new irepBoletoInmuSM((short) 2);
    DTOBoletoInmuSM dtoBoletoInmuSm = new DTOBoletoInmuSM();
    List<DTOBoletoInmuSM> dtoBoletoInmuSmList = new List<DTOBoletoInmuSM>();
    dtoBoletoInmuSm.PeriBole = PARAMS.Periodo;
    dtoBoletoInmuSm.CucuPers = "";
    dtoBoletoInmuSmList.Add(dtoBoletoInmuSm);
    ((XtraReportBase) bole).DataSource = (object) dtoBoletoInmuSmList;
    bole.createReport((Action<int, string>) null, false);
    SaveFileDialog dialog = new SaveFileDialog();
    dialog.AddExtension = true;
    dialog.AutoUpgradeEnabled = true;
  ...
  {
    this.ExportarPdfBut.Enabled = this.btnGenerarFijo.Enabled = this.butGenerarPdfEmail.Enabled = false;
    List<DTOBoletoInmuSM> listaRepo = this.buscarLiquiedacionInmu();
  ...
  public void ExportarEmailgPdf(List<DTOBoletoInmuSM> listaRepo, string rutaFinal)
  {
    foreach (DTOBoletoInmuSM dtoBoletoInmuSm in listaRepo)
    {
      irepBoletoInmuSM irepBoletoInmuSm = new irepBoletoInmuSM((short) 0);
      ((XtraReportBase) irepBoletoInmuSm).DataSource = (object) new List<DTOBoletoInmuSM>()
      {
        dtoBoletoInmuSm
      };
      irepBoletoInmuSm.createReport((Action<int, string>) null, false);
      PdfExportOptions pdfExportOptions = new PdfExportOptions();
      irepBoletoInmuSm.ExportToPdf($"{rutaFinal}\\1-{dtoBoletoInmuSm.CodiInmu.ToString()}.pdf", (PdfExportOptions) null);
    }
  }

  private List<DTOBoletoInmuSM> buscarLiquiedacionInmu()
  {
    List<DTOBoletoInmuSM> listaRepo = new List<DTOBoletoInmuSM>();
    if (this.validar())
      return listaRepo;
    EmisionMasivaTasas row = (EmisionMasivaTasas) ((BaseView) this.gridView1).GetRow(((ColumnView) this.gridView1).GetSelectedRows()[0]);
    ...
    List<DTOBoletoInmuSM> dtoBoletoInmuSmList = new frmEsperar((Form) this).calcular<List<DTOBoletoInmuSM>>((Func<List<DTOBoletoInmuSM>>) (() => this.experto.obtenerTasasSM(row, this.DesdeText.IntText, this.HastaText.IntText)), "Consultado Datos...");
    int num = 0;
    int index = 0;
    foreach (DTOBoletoInmuSM tasa in dtoBoletoInmuSmList)
    {
      if (tasa.CodiInmu != num)
      {
        num = tasa.CodiInmu;
        listaRepo.Add(tasa);
        listaRepo[index].cupones = new List<DTOBoletoInmuSM.CuponInmuSM>();
        listaRepo[index].cupones.Add(this.experto.obtenerCuponSM(row, tasa, tasa.TipoVenc == 2));
        ++index;
      }
      else
        listaRepo[index - 1].cupones.Add(this.experto.obtenerCuponSM(row, tasa, tasa.TipoVenc == 2));
    }
  ...
  {
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (frmEmisionMasivaTasasSM));
    ...
    this.separator1.Size = new Size(10743, 2);
    ...
    this.butGenerarPdfEmail.TabIndex = 10;
    ...
    this.Name = nameof (frmEmisionMasivaTasasSM);
