...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.frmEmisionMasivaCementerioGUAY
...
public class frmEmisionMasivaCementerioGUAY : FRMForm
{
  private BindingSource bsEmison = new BindingSource();
  private ExpEmisionMasivaCementerioGUAY experto = new ExpEmisionMasivaCementerioGUAY();
  ...
  public frmEmisionMasivaCementerioGUAY()
    : base("Emision masiva de Cementerio", "Emision masiva de Cementerio", typeof (ExpEmisionMasivaCementerioGUAY))
  {
  ...
  public List<List<DTOBoletoCementerioGuay>> splitList<DTOBoletoCementerioGuay>(
    List<DTOBoletoCementerioGuay> locations,
    int nSize = 250)
  {
    List<List<DTOBoletoCementerioGuay>> boletoCementerioGuayListList = new List<List<DTOBoletoCementerioGuay>>();
    for (int index = 0; index < locations.Count; index += nSize)
      boletoCementerioGuayListList.Add(locations.GetRange(index, Math.Min(nSize, locations.Count - index)));
    return boletoCementerioGuayListList;
  }

  public List<List<DTOBoletoCementerioGuay>> splitListxCupones(
    List<DTOBoletoCementerioGuay> locations,
    int nSize = 250)
  {
    List<List<DTOBoletoCementerioGuay>> boletoCementerioGuayListList = new List<List<DTOBoletoCementerioGuay>>();
    ...
    for (int index = 0; index < locations.Count; index += val1)
      boletoCementerioGuayListList.Add(locations.GetRange(index, Math.Min(val1, locations.Count - index)));
    return boletoCementerioGuayListList;
  }
  ...
  private void ExportarPdfBut_Click(object sender, EventArgs e)
  {
    ...
    else
    {
      ...
      List<DTOBoletoCementerioGuay> boletoCementerioGuayList1 = new frmEsperar((Form) this).calcular<List<DTOBoletoCementerioGuay>>((Func<List<DTOBoletoCementerioGuay>>) (() => this.experto.obtenerLiquCementerio(row, this.DesdeText.IntText, this.HastaText.IntText)), "Consultado Datos...");
      List<DTOBoletoCementerioGuay> boletoCementerioGuayList2 = new List<DTOBoletoCementerioGuay>();
      int num = 0;
      int index = 0;
      foreach (DTOBoletoCementerioGuay boletoCementerioGuay in boletoCementerioGuayList1)
      {
        if (boletoCementerioGuay.CodiCome != num)
        {
          num = boletoCementerioGuay.CodiCome;
          boletoCementerioGuayList2.Add(boletoCementerioGuay);
          boletoCementerioGuayList2[index].cupones = new List<CuponCementerio>();
          ++index;
        }
      }
      this.agregarQrVinculado(boletoCementerioGuayList2);
      if (boletoCementerioGuayList2.Count<DTOBoletoCementerioGuay>() > 250)
      {
        List<List<DTOBoletoCementerioGuay>> boletoCementerioGuayListList = this.splitList<DTOBoletoCementerioGuay>(boletoCementerioGuayList2);
        FolderBrowserDialog dialog = new FolderBrowserDialog();
        if (dialog.ShowDialog((IWin32Window) this) == DialogResult.OK)
        {
          foreach (List<DTOBoletoCementerioGuay> boletoCementerioGuayList3 in boletoCementerioGuayListList)
          {
            List<DTOBoletoCementerioGuay> l = boletoCementerioGuayList3;
            irepBoletoComenterioGuay bole = new irepBoletoComenterioGuay();
            ((XtraReportBase) bole).DataSource = (object) l;
            bole.createReport((Action<int, string>) null, false);
            PdfExportOptions pdfExportOptions = new PdfExportOptions();
            new frmEsperar((Form) this).procesar((Action) (() => bole.ExportToPdf($"{dialog.SelectedPath}\\cementerio ({l[0].OrdeLico.ToString()}-{l[l.Count - 1].OrdeLico.ToString()}).pdf", (PdfExportOptions) null)), "Exportando..");
          }
        }
      ...
      {
        irepBoletoComenterioGuay bole = new irepBoletoComenterioGuay();
        ((XtraReportBase) bole).DataSource = (object) boletoCementerioGuayList2;
        bole.createReport((Action<int, string>) null, false);
  ...
  public void agregarQrVinculado(List<DTOBoletoCementerioGuay> listaRepo)
  {
    foreach (DTOBoletoCementerioGuay boletoCementerioGuay in listaRepo)
    {
      string str1 = "";
      foreach (CuponCementerio cupone in boletoCementerioGuay.cupones)
        str1 = $"{str1}{cupone.NumeBole.ToString()}-";
      string str2 = str1.TrimEnd('-');
      boletoCementerioGuay.EpagosQrVinc = $"https://vps.infogov.com.ar/rentas/guaymallen/V1/index.php/api/epagos/opcionespago/{str2}/{boletoCementerioGuay.PeriBole.ToString()}";
    }
  }
  ...
  private void InitializeComponent()
  {
    ...
    this.separator1.Size = new Size(9859, 2);
    ...
    this.Name = "frmEmisionMasivaComercioGUAY";
