...
// Type: InfoGov.Plugins.Recaudacion.Ventanas.frmIngresoBoletoGrupoMP
...
public class frmIngresoBoletoGrupoMP : FRMForm
{
  private ExpBoletoGrupoMP expGrupo = new ExpBoletoGrupoMP();
  ...
  private MetroTextBox txtNuchFopa;

  public frmIngresoBoletoGrupoMP()
    : base("Ingreso de Boletos por Grupo", "Boletos por Grupo", typeof (BoletoGrupoMP))
  {
  ...
  {
    this.dtpFechaEnvio.MaxDate = this.dtpFechaPago.MaxDate = this.hoy.Date;
  ...
  {
    this.dtpFechaEnvio.Value = this.dtpFechaPago.Value = this.hoy.Date;
    ...
    this.txtNuchFopa.Text = string.Empty;
    this.mtextPeriBole.IntText = this.hoy.Year;
  }

  private void CompletarFormulario(BoletoGrupoMP grupo)
  {
    this.dtpFechaEnvio.Value = grupo.FeenAcpa.Date;
    this.dtpFechaPago.Value = grupo.FepaAcpa.Date;
  ...
  {
    this.mtextCodiBarra.Enabled = this.mtextPeriBole.Enabled = this.mtextNumeBole.Enabled = this.mcboFormaPago.Enabled = this.btnAgregar.Enabled = this.btnAgregarMulti.Enabled = this.txtNuchFopa.Enabled = activar;
  }
  ...
  {
    int[] selectedRows = ((ColumnView) this.viewGrupos).GetSelectedRows();
    if (selectedRows.Length == 0)
      return;
    BoletoGrupoMP row1 = ((BaseView) this.viewGrupos).GetRow(selectedRows[0]) as BoletoGrupoMP;
    this.LlenarGridGrupos();
    this.FocusFilaInvalidaGrupo();
    if (row1 == null)
      return;
    for (int index = 0; index < ((BaseView) this.viewGrupos).RowCount; ++index)
    {
      if (((BaseView) this.viewGrupos).GetRow(index) is BoletoGrupoMP row2 && row2.FeenAcpa.Date == row1.FeenAcpa.Date && row2.NumeAcpa == row1.NumeAcpa)
      {
        ((ColumnView) this.viewGrupos).FocusedRowHandle = index;
        ((ColumnView) this.viewGrupos).SelectRow(index);
        break;
      }
    }
  }
  ...
  private void AgregarBoleto(bool codiBar = false)
  {
    ...
    frmIngresoBoletoGrupoMP.\u003C\u003Ec__DisplayClass16_0 cDisplayClass160;
    ...
    {
      ref frmIngresoBoletoGrupoMP.\u003C\u003Ec__DisplayClass16_0 local = ref cDisplayClass160;
      frmEsperar frmEsperar = new frmEsperar((Form) this);
      frmEsperar.Mensaje = "Verificando Datos Boleto...";
      frmEsperar.Width = 320;
      // ISSUE: reference to a compiler-generated field
      local.esperar = frmEsperar;
      try
      {
        this.Enabled = false;
        // ISSUE: reference to a compiler-generated field
        cDisplayClass160.esperar.SoloEsperar();
        Application.DoEvents();
        string NuchFopa = this.txtNuchFopa.Text.Trim();
        string empty = string.Empty;
        string msg1 = this.expGrupo.CajaVerificada(this.dtpFechaEnvio.Value, this.mibGrupo.ShortText);
        if (!string.IsNullOrWhiteSpace(msg1))
        {
          // ISSUE: reference to a compiler-generated method
          this.\u003CAgregarBoleto\u003Eg__MostrarError\u007C16_0(msg1, ref cDisplayClass160);
          this.FocusControlIngresoPago();
        }
        else
        {
          string msg2 = this.expGrupo.GrupoActualizado(this.dtpFechaEnvio.Value, this.mibGrupo.IntText);
          if (!string.IsNullOrWhiteSpace(msg2))
          {
            // ISSUE: reference to a compiler-generated method
            this.\u003CAgregarBoleto\u003Eg__MostrarError\u007C16_0(msg2, ref cDisplayClass160);
            this.FocusControlIngresoPago();
          }
          else
          {
            string msg3 = this.expGrupo.VerificarBoleto(this.mtextPeriBole.IntText, this.mtextNumeBole.IntText);
            if (!string.IsNullOrWhiteSpace(msg3))
            {
              // ISSUE: reference to a compiler-generated method
              this.\u003CAgregarBoleto\u003Eg__MostrarError\u007C16_0(msg3, ref cDisplayClass160);
              this.FocusControlIngresoPago();
            }
            else
            {
              string msg4 = this.expGrupo.VerificarPago(this.mtextPeriBole.IntText, this.mtextNumeBole.IntText);
              if (!string.IsNullOrWhiteSpace(msg4))
              {
                // ISSUE: reference to a compiler-generated method
                this.\u003CAgregarBoleto\u003Eg__MostrarError\u007C16_0(msg4, ref cDisplayClass160);
                this.FocusControlIngresoPago();
              }
              else
              {
                string msg5 = this.expGrupo.Pagado(this.mtextPeriBole.IntText, this.mtextNumeBole.IntText);
                if (!string.IsNullOrWhiteSpace(msg5))
                {
                  // ISSUE: reference to a compiler-generated method
                  this.\u003CAgregarBoleto\u003Eg__MostrarError\u007C16_0(msg5, ref cDisplayClass160);
                  this.FocusControlIngresoPago();
                }
                else
                {
                  // ISSUE: reference to a compiler-generated field
                  cDisplayClass160.esperar.Mensaje = "Ingresando Boleto...";
                  Application.DoEvents();
                  string mensaje = this.expGrupo.IngresarBoleto(this.dtpFechaEnvio.Value, this.mibGrupo.IntText, this.mtextPeriBole.IntText, this.mtextNumeBole.IntText, (int) this.mcboFormaPago.SelectedValue, NuchFopa, this.dtpFechaPago.Value);
      ...
      {
        // ISSUE: reference to a compiler-generated field
        cDisplayClass160.esperar.Close();
        // ISSUE: reference to a compiler-generated field
        cDisplayClass160.esperar.Dispose();
        this.Enabled = true;
      }
    }
  }
  ...
  protected override void butDelete_Click(object sender, EventArgs e)
  {
    ...
    this.ActivarIngresoBoleto(false);
    this.ActivarGrids(false);
    new frmMensaje((Form) this).mostrarInformación("Al eliminar un grupo, se eliminarán TODOS LOS PAGOS ASOCIADOS.");
  }
  ...
  {
    if (e.KeyCode == Keys.Back || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Home || e.KeyCode == Keys.End || e.KeyCode == Keys.Left || e.KeyCode == Keys.Right || e.KeyCode == Keys.Up || e.KeyCode == Keys.Down)
  ...
  private void viewGrupos_SelectionChanged(object sender, SelectionChangedEventArgs e)
  {
    ...
    {
      this.LimpiarControles();
      BoletoGrupoMP row = (BoletoGrupoMP) ((BaseView) this.viewGrupos).GetRow(((ColumnView) this.viewGrupos).GetSelectedRows()[0]);
    ...
    {
      this.ActivarIngresoBoleto(false);
      this.gridBoletos.DataSource = (object) new List<BoletoCajaMP>();
      ((Control) this.gridBoletos).Refresh();
    }
  }
  ...
  {
    if (!this.hasPermissionFor("EliminarPagos"))
    {
      new frmMensaje((Form) this).mostrarError("NO TIENE PERMISOS PARA ELIMINAR PAGOS.");
    }
    else
    {
      BoletoCajaMP focusedRow = (BoletoCajaMP) ((ColumnView) this.viewBoletos).GetFocusedRow();
      BoletoGrupoMP row = (BoletoGrupoMP) ((BaseView) this.viewGrupos).GetRow(((ColumnView) this.viewGrupos).GetSelectedRows()[0]);
      ...
      else
      {
        ...
        switch (num)
        {
          ...
          case 3:
          case 4:
            if (new frmMostrarFopaMP(row.FeenAcpa, row.NumeAcpa, focusedRow.PeriBole, focusedRow.NumeBole, focusedRow.TotaBole, focusedRow.PeriFopa, focusedRow.NumeFopa, num).ShowDialog() == DialogResult.OK)
            {
              this.ResetearGrupoActual();
  ...
  {
    if (this.mtextCodiBarra.Text.Trim().Length == this.mtextCodiBarra.MaxLength)
    {
  ...
  private void frmIngresoBoletoGrupoMP_Activated(object sender, EventArgs e)
  {
    this.ResetearGrupoActual();
  }
  ...
  {
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (frmIngresoBoletoGrupoMP));
    ...
    this.txtNuchFopa = new MetroTextBox();
    ...
    this.separator1.Size = new Size(20644, 2);
    ...
    this.colInteBole.VisibleIndex = 10;
    ...
    this.colExenBole.VisibleIndex = 11;
    ...
    this.colApreBole.VisibleIndex = 12;
    ...
    this.colTotaBole.VisibleIndex = 13;
    ...
    this.colEliminarBoleto.VisibleIndex = 14;
    ...
    this.panAgregarBoleto.Controls.Add((Control) this.txtNuchFopa);
    ...
    this.txtNuchFopa.AllowEmpty = true;
    this.txtNuchFopa.Location = new Point(639, 23);
    this.txtNuchFopa.MaxLength = 50;
    this.txtNuchFopa.Name = "txtNuchFopa";
    this.txtNuchFopa.PromptText = "Observación";
    this.txtNuchFopa.Size = new Size(235, 23);
    this.txtNuchFopa.TabIndex = 19;
    ...
    this.btnAgregarMulti.Visible = false;
    ...
    this.mibGrupo.MaxLength = 3;
    this.mibGrupo.MaxValue = 999;
    ...
    this.Name = nameof (frmIngresoBoletoGrupoMP);
    this.Text = "Ingreso de Boletos";
    this.Activated += new EventHandler(this.frmIngresoBoletoGrupoMP_Activated);
