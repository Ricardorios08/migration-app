...
// Type: InfoGov.Plugins.Recaudacion.Expertos.ExpBoletoCajaGuay
...
internal class ExpBoletoCajaGuay : Expert
{
  ...
  public void grupoControlado(DateTime fecha)
  {
    if (this.conn.executeInt($"SELECT COUNT(*) FROM acrepago WHERE FeenAcpa = {this.sqlDate(fecha, false)} AND NumeAcpa = {this.uo.NumeAcpa} AND ContAcpa = 1 LIMIT 1;") > 0)
      throw new Exception("El grupo ya se encuentra controlado... Verifique...");
  }

  public string ControlarNumerosOperacion(string Numeros)
  {
    string empty = string.Empty;
    string str = this.conn.executeString($"SELECT IFNULL(GROUP_CONCAT(DISTINCT NuchFopa),'') FROM formpago WHERE NuchFopa IN ({Numeros});");
    if (!string.IsNullOrEmpty(str))
      str = $"Ya se han ingreso los siguientes números de operacion: {str}. Verificar..";
    return str;
  }
  ...
  public void ingresarPago(DateTime fecha, List<BoletoCaja> boletos, List<FormaPago> formasPago)
  {
    ...
    this.grupoControlado(fecha);
    StringBuilder stringBuilder = new StringBuilder();
    if (this.cantAcrePago(fecha) == 0)
      stringBuilder.AppendLine($" INSERT INTO acrepago (FeenAcpa,NumeAcpa,ActuAcpa,CodiEnre,FealAcpa,FepaAcpa,TocoAcpa,ImpoAcpa,ObseAcpa,ContAcpa,CodiUsua) VALUES ({this.sqlDate(fecha, false)},{this.uo.NumeAcpa} ,'No',{this.uo.CodiEnre},{this.sqlDate(DateTime.Now, false)},{this.sqlDate(fecha, false)},{boletos.Count},{this.sqlDecimal(boletos.Sum<BoletoCaja>((Func<BoletoCaja, Decimal>) (x => x.TotaBole)))},'',0,{this.sqlString(this.usuario)});");
    else
    ...
    foreach (FormaPago formaPago in formasPago)
      stringBuilder.AppendLine($"INSERT INTO formpago (PeriFopa,NumeFopa,MoviFopa, CodiTipa,NuchFopa,ImpoFopa) VALUES ({PARAMS.Periodo},{num},{formasPago.IndexOf(formaPago) + 1}, {formaPago.CodiTipa},{this.sqlString(formaPago.NuchFopaStr)},{this.sqlDecimal(formaPago.ImpoFopa)});");
