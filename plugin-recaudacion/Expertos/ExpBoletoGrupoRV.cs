...
// Type: InfoGov.Plugins.Recaudacion.Expertos.ExpBoletoGrupoRV
...
internal class ExpBoletoGrupoRV : Expert
{
  ...
  public List<BoletoCaja> BuscarBoletosIngresados(DateTime fechaEnvio, int grupo)
  {
    ...
    {
      while (nfoGovReader.Read())
        boletoCajaList.Add(new BoletoCaja()
        {
    ...
    DateTime FechPago)
  {
    ...
    BoletoCaja boletoCaja = this.ObtenerBoleto(PeriBole, Numebole);
    List<DetalleBoleto> detalleBoletoList = this.ObtenerDetallesBoleto(boletoCaja);
    stringBuilder.AppendLine($"UPDATE acrepago SET TocoAcpa = TocoAcpa + 1, ImpoAcpa = ImpoAcpa + {this.sqlDecimal(boletoCaja.TotaBole)} WHERE FeenAcpa = {this.sqlDate(FeenAcpa, false)} AND NumeAcpa = {NumeAcpa};");
    ...
    stringBuilder.AppendLine($"INSERT INTO formpago (PeriFopa,NumeFopa,MoviFopa,CodiTipa,NuchFopa,ImpoFopa) VALUES ({PARAMS.Periodo},{num},1,{CodiTipa},{NuchFopa},{this.sqlDecimal(boletoCaja.TotaBole)});");
    ...
    stringBuilder.AppendLine($" VALUES ({this.sqlDate(FeenAcpa, false)},{NumeAcpa},{boletoCaja.PeriBole},{boletoCaja.NumeBole},{PARAMS.Periodo},{num},{boletoCaja.CodiOfic},{this.sqlString(boletoCaja.CuenCtct)},{boletoCaja.CucuPers},'No',{this.sqlString(boletoCaja.TipoBole)},{this.sqlDate(DateTime.Now, false)},{this.sqlDate(FechPago, false)},{boletoCaja.CodiFapa},{boletoCaja.CuotDefa},{this.sqlDecimal(boletoCaja.CapiBole)},{this.sqlDecimal(boletoCaja.DecaBole)},{this.sqlDecimal(boletoCaja.RecaBole)},{this.sqlDecimal(boletoCaja.DereBole)},{this.sqlDecimal(boletoCaja.InteBole)},{this.sqlDecimal(boletoCaja.ExenBole)},{this.sqlDecimal(boletoCaja.CrafBole)},{this.sqlDecimal(boletoCaja.ApreBole)},{this.sqlDecimal(boletoCaja.TotaBole)},{this.sqlString(this.usuario)});");
    if (FechPago.Date > boletoCaja.FeveBole.Date)
      stringBuilder.AppendLine($"INSERT INTO recabole (CodiOfic,CuenCtct,PeriBole,NumeBole,ActuRebo,FealRebo,FeveRebo,ImpoRebo) VALUES ({boletoCaja.CodiOfic},{this.sqlString(boletoCaja.CuenCtct)},{boletoCaja.PeriBole},{boletoCaja.NumeBole},'No',{this.sqlDate(DateTime.Now, false)},{this.sqlDate(boletoCaja.FeveBole, false)},{this.sqlDecimal(boletoCaja.TotaBole)});");
    if (this.conn.executeBool($" SELECT IF(NumeApre IS NOT NULL, TRUE, FALSE) FROM boleapre WHERE PeriBole = {boletoCaja.PeriBole} AND NumeBole = {boletoCaja.NumeBole} AND EstaBoap IN ('Generado','Emitido') LIMIT 1;"))
      stringBuilder.AppendLine($" UPDATE boleapre SET EstaBoap = 'Pagado', FeenAcpa = {this.sqlDate(FeenAcpa, false)}, FeacBoap = NULL WHERE PeriBole = {boletoCaja.PeriBole} AND NumeBole = {boletoCaja.NumeBole} AND EstaBoap IN ('Generado','Emitido');");
    stringBuilder.AppendLine($"UPDATE boleto SET EstaBole = 'Pagado' WHERE PeriBole = {boletoCaja.PeriBole} AND NumeBole = {boletoCaja.NumeBole};");
    foreach (DetalleBoleto dtb in detalleBoletoList)
    {
      ...
      if (boletoCaja.TipoBole == "Pago Anticipado")
    ...
    if (this.tickeadoraActiva == "True")
      this.expImpTicket.ImprimirTicket(boletoCaja);
    return true;
  }
  ...
  {
    return this.conn.executeNonQuery($"INSERT INTO acrepago(FeenAcpa,NumeAcpa,ActuAcpa,CodiEnre,FealAcpa,FepaAcpa,CodiUsua) VALUES({this.sqlDate(FeenAcpa, false)},{NumeAcpa},'No',{CodiEnre},{this.sqlDate(DateTime.Now, false)},{this.sqlDate(FepaAcpa, false)},{this.sqlString(this.usuario)});");
  }
